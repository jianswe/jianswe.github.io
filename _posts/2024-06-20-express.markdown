...
## Error Handlers
Error handlder has to come after all the routes. 
`src/server.ts`
```ts
app.post('/signin', signin)
...
app.use((err, req, res, next) => {
  console.log(err)
  res.json({message: 'opps there was an error'})
})
...
Express cannot handle async errors. The server will crash. We need to handle async errors by ourselves. 
```ts
